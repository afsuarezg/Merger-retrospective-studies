...
def creating_product_data_rcl(main_dir: str,
    ...
    products_info_data = products_file(products_path=products_path)
    ...
    # Combina los datos
    product_data = pd.merge(movements_data, stores_data, on='store_code_uc', how='left')
    product_data = pd.merge(product_data, products_info_data, on='upc', how='left')
    product_data = pd.merge(product_data, extra_attributes_data, on='upc', how='left')
    ...
    #-------------------------------------------------------------------
    # Determinar porción de ventas identificadas para cada tienda a través de ingresos  (recordar que los mercados se definieron como la combinación entre tienda y semana). 
    ...
    total_sales_per_marketid.rename(columns={'total_income':'total_income_per_market_id'}, inplace=True)
    ...
    total_sales_identified_per_marketid.rename(columns={'total_income':'total_income_per_market_id_known_brands'}, inplace=True)
    ...
    product_data.fillna({'total_income_per_market_id_known_brands': 0.0}, inplace=True)
    
    #Calcula y agrega columna con el cálculo de la fraccion de ventas identificadas
    ...
    # TODO Revisar que las participaciones de mercado estimadas del modelo sean cercanas a las observadas en otros estudios. Aunque es difícil que ocurra porque no se tiene toda la demanda por ubicación geográfica. Esto no se va a verificar porque como solo se están analizando algunos retailers, es difícil calcular el consumo a nivel de mercado únicamente con los datos de retailers aislados.
    ...
    #-------------------------------------------------------------------
    # Agrega características de los productos al dataframe de product data
    ...
    #-------------------------------------------------------------------
    # Genera indicador numérico para los markets_ids
